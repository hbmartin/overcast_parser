...
class Item:
    ...
    def __str__(self):
        return f"{self.guid} {self.title} ({self.enclosure_url})"
    ...
    def to_dict(self):
        item = {
            ...
            "itunes_closed_captioned": self.itunes_closed_captioned,
    ...
    def set_itunes_duration(self):
        ...
        try:
            self.itunes_duration = int(self.soup.find("itunes:duration").string)
        except AttributeError:
    ...
    def set_itunes_keywords(self):
        """ Parse keywords from itunes tags and set value"""
        try:
